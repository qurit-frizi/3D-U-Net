...
def main(args):
    ...
    output_transforms = [
        ...
        transforms.CheckOutputShape(shape=(144,144,144)) #size of your resampled data
    ]
    if not probs:
    ...
    data_set = dataset.PETCTDataset(all_paths, transforms=input_transforms, mode='test')
    data_loader = DataLoader(data_set, batch_size=1, shuffle=False, num_workers=num_workers)

    # model:
    model = models.FastSmoothSENormDeepUNet_supervision_skip_no_drop(in_channels, n_cls, n_filters, reduction) #you can select U-Net
